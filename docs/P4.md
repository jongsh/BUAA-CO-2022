...
* ### **1.** *阅读下面给出的 DM 的输入示例中（示例 DM 容量为 4KB，即 32bit × 1024字），根据你的理解回答，这个 addr 信号又是从哪里来的？地址信号 addr 位数为什么是 [11:2] 而不是 [9:0] ？*
  ### <font color=red>DM 的 addr 信号来自寄存器文件读取的数据与符号扩展后的立即数在 ALU 模块做计算得到的结果。
  ### 在我们的设计中，DM 的每一个存储单元都是32位的，即4个字节，而 MARS 中是按字节编址，如果只考虑 lw,sw 指令，我们对 DM 数据则是按字处理，对设计中的寄存器是按字编码，因此会舍去低两位。</font>
  <br>
...
* ### **2.** *思考上述两种控制器设计的译码方式，给出代码示例，并尝试对比各方式的优劣。*
  ...
  <br>
      ...
      第二种方式：
      assign RegWrite = add || sub || lw ...;
      assign MemWrite = sw;
      //...
  ### <font color=red>对于第一种方式，代码量比较冗长，但一旦出现bug非常容易定位查找；第二种方式比较简洁，适合增量开发，但很容易写漏某些信号导致错误且不易找出。</font>
  <br>

* ### **3.** *在相应的部件中，复位信号的设计都是同步复位，这与 P3 中的设计要求不同。请对比同步复位与异步复位这两种方式的 reset 信号与 clk 信号优先级的关系。*
  ### <font color=red>异步复位中，reset 信号的优先级高于 clk 信号；在同步复位中，clk 信号的优先级高于 reset 信号。</font>
  <br>
...
* ### **4.** *C 语言是一种弱类型程序设计语言。C 语言中不对计算结果溢出进行处理，这意味着 C 语言要求程序员必须很清楚计算结果是否会导致溢出。因此，如果仅仅支持 C 语言，MIPS 指令的所有计算指令均可以忽略溢出。 请说明为什么在忽略溢出的前提下，addi 与 addiu 是等价的，add 与 addu 是等价的。*
  ### <font color=red>add 和 addi 通过增加一位符号位来判断是否溢出。如果不溢出，即计算结果的双符号位一致，则把低32位写入目标寄存器。当不考虑溢出时，这一结果和addu、addiu 是一致的，因此等价。</font>